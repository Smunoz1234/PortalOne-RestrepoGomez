    ...
    while ($row = sqlsrv_fetch_array($SQL)) {
		...
		<tr id="<?php echo $i; ?>" onClick="Resaltar('<?php echo $i; ?>');">
			...
			<td class="text-center form-inline w-80">
				<!-- SMM, 10/01/2022 -->
				<?php $linea = ["SucursalCliente" => $row["IdSucursalCliente"], "Estado" => $row["Estado"], "Frecuencia" => $row["Frecuencia"], "IdArticulo" => $row["IdArticuloLMT"], "DeArticulo" => $row["DeArticuloLMT"], "Areas" => $row["Areas"], "Servicios" => $row["Servicios"], "MetodoAplicacion" => $row["MetodoAplicacion"]];?>
				<button type="button" title="Más información" class="btn btn-info btn-xs" onClick="InfoLinea(<?php echo str_replace('"', "'", json_encode($linea)); ?>);"><i class="fa fa-info"></i></button>
...
<script>
	...
	function InfoLinea(linea) {
		$("#SucursalCliente").val(linea.SucursalCliente);
