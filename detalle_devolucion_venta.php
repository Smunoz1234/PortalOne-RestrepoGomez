...
function Totalizar(num, totalizar=true) {
	...
	window.parent.document.getElementById('TotalDevolucion').value = number_format(Math.floor(Math.round(Total)), dPrecios);
...
function ActualizarDatos(name, id, line, round=0) { // Actualizar datos asincronicamente
	...
	$.ajax({
		type: "GET",
		<?php if ($type == 1) {?>
		url: "registro.php?P=36&doctype=7&type=1&name="+name+"&value="+Base64.encode(valor)+"&line="+line+"&cardcode=<?php echo $CardCode; ?>&whscode=<?php echo $Almacen; ?>&actodos=0",
		<?php } else {?>
		url: "registro.php?P=36&doctype=7&type=2&name="+name+"&value="+Base64.encode(valor)+"&line="+line+"&id=<?php echo base64_decode($_GET['id']); ?>&evento=<?php echo base64_decode($_GET['evento']); ?>&actodos=0",
		<?php }?>
		success: function(response){
			if(response!="Error"){
