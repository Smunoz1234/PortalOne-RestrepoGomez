					...
					<div class="col-lg-3">
						<div class="btn-group pull-right">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="alkin dropdown-item" href="devolucion_venta.php?dt_ET=1&Cardcode=<?php echo base64_encode($row['CardCode']);?>&Dim2=<?php echo base64_encode($row['OcrCode2']);?>&Dim1=<?php echo base64_encode($row['OcrCode']);?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']);?>&Direccion=<?php echo base64_encode($row['DireccionDestino']);?>&Almacen=<?php echo base64_encode($row['WhsCode']);?>&Contacto=<?php echo base64_encode($row['CodigoContacto']);?>&Referencia=<?php echo base64_encode($row['NumAtCard']);?>&Empleado=<?php echo base64_encode($row['SlpCode']);?>&ET=<?php echo base64_encode($row['ID_EntregaVenta']);?>&Evento=<?php echo base64_encode($row['IdEvento']);?>&dt_LS=1&LS=<?php echo base64_encode($row['ID_LlamadaServicio']);?>">Devolución de venta</a></li>
                            </ul>
                        </div>
					</div>
					<?php }?>
