				...
				<div class="row">
					<div class="col-lg-12">
						<div class="ibox-content">
							...
							<form action="impresion_orden_servicio.php" method="get" id="formBuscar"
								...
								<div class="form-group">
									...
									<label class="col-lg-1 control-label"><select id="TipoFecha" name="TipoFecha">
											<option value="FechaCreacionLLamada" <?php if (isset($_GET['TipoFecha']) && ($_GET['TipoFecha'] == "FechaCreacionLLamada")) {
												echo "selected";
											} ?>>Fecha Creación</option>
											<option value="FechaCierreLLamada" <?php if (isset($_GET['TipoFecha']) && ($_GET['TipoFecha'] == "FechaCierreLLamada")) {
												echo "selected";
											} ?>>Fecha Cierre</option>
											<option value="FechaAgenda" <?php if (isset($_GET['TipoFecha']) && ($_GET['TipoFecha'] == "FechaAgenda")) {
												echo "selected";
											} ?>>Fecha Agenda</option>
										</select></label>
									<div class="col-lg-3">
								...
								<div class="form-group">
									...
									<div class="col-lg-3">
										<input name="Servicio" type="text" class="form-control" id="Servicio"
											maxlength="50"
											value="<?php if (isset($_GET['Servicio']) && ($_GET['Servicio'] != "")) {
												echo $_GET['Servicio'];
											} ?>">
									</div>
								</div>
								...
								<div class="form-group">
									<div class="col-lg-8">
										<?php if ($sw == 1) { ?>
											<a
												href="exportar_excel.php?exp=10&Cons=<?php echo base64_encode(implode(",", $Param)); ?>&sp=<?php echo base64_encode('sp_ConsultarImpresionOT'); ?>">
					...
					<div class="row">
						<div class="col-lg-12">
							<div class="ibox-content">
								...
								<div class="form-group">
									<label class="col-xs-12">
										<h3 class="bg-success p-xs b-r-sm"><i class="fa fa-check-square"></i> Datos para
											seleccionar</h3>
									</label>
								</div>
								<div class="row m-b-md">
									<div class="col-lg-6">
										<form id="frmDownload" name="frmDownload" action="attachdownload.php" method="post"
											target="_blank">
											<input type="hidden" id="file" name="file" value="" />
											<input type="hidden" id="zip" name="zip"
												value="<?php echo base64_encode('1'); ?>" />
										</form>
									</div>
									<div class="col-lg-6">
										<div class="btn-group pull-right">
											<button data-toggle="dropdown" class="btn btn-primary disabled dropdown-toggle"
												id="btnZIP" name="btnZIP"><i class="fa fa-download"></i> Descargar <i
													class="fa fa-caret-down"></i></button>
											<ul class="dropdown-menu">
												<li>
													<a class="dropdown-item" href="javascript:void(0);"
														onClick="DescargarANX();">Anexos</a>
													<?php
													...
													while ($row_Formato = sqlsrv_fetch_array($SQL_Formato)) {
														...
														<a class="dropdown-item" href="javascript:void(0);"
															onClick="DescargarZIP('<?php echo $row_Formato['ID']; ?>');"><?php echo $Nombre; ?></a>
													<?php } ?>
												</li>
											</ul>
										</div>
											...
											<tbody>
												...
												while ($row = sql_fetch_array($SQL)) {
													...
													<tr id="tr_<?php echo $i; ?>" class="gradeX">
														<td><a href="llamada_servicio.php?id=<?php echo base64_encode($row['ID_LlamadaServicio']); ?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('impresion_orden_servicio.php'); ?>&tl=1"
														...
														} ?>><?php echo $row['DeEstadoLlamada']; ?></span>
														</td>
														<td><span <?php if ($row['CDU_EstadoServicio'] == '0') {
															echo "class='label label-warning'";
														} elseif ($row['CDU_EstadoServicio'] == '1') {
															echo "class='label label-primary'";
														} else {
															echo "class='label label-danger'";
														} ?>><?php echo $row['DeEstadoServicio']; ?></span>
														</td>
														<td><?php if ($row['DocNumFactura'] != "") { ?><a
																	href="factura_venta.php?id=<?php echo base64_encode($row['DocEntryFactura']); ?>&id_portal=<?php echo base64_encode($row['DocPortalFactura']); ?>&tl=1&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('impresion_orden_servicio.php'); ?>"
																	target="_blank"><?php echo $row['DocNumFactura']; ?></a><?php } else {
															echo "NO";
														} ?>
														</td>
														<td><?php if ($row['DeAnexoLlamada'] != "") { ?><a
																	href="attachdownload.php?file=<?php echo base64_encode($row['IdAnexoLlamada']); ?>&line=<?php echo base64_encode($row['LineNumAnexoLlamada']); ?>"
																	target="_blank" title="Descargar archivo"
																	class="btn-link btn-xs"><i class="<?php echo $Icon; ?>"></i>
																	<?php echo $row['DeAnexoLlamada']; ?></a><?php } ?></td>
														<td>
															<?php //if($row['DeAnexoLlamada']!=""){ ?>
															<div class="checkbox checkbox-success">
																<input type="checkbox" class="chkSelOT"
																	id="chkSelOT<?php echo $row['DocNum']; ?>" value=""
																	onChange="SeleccionarOT('<?php echo $row['DocNum']; ?>','<?php echo $row['IdAnexoLlamada']; ?>','<?php echo $row['LineNumAnexoLlamada']; ?>','<?php echo $row['ID_LlamadaServicio']; ?>','<?php echo $row['Series']; ?>');"
