...
<script type="text/javascript">
	$(document).ready(function() { // Cargar los combos dependiendo de otros
		$("#CardCode").change(function() {
			...
			<?php if ($edit == 0 && $sw_error == 0 && $dt_LS == 0 && $dt_ET == 0) { // Limpiar carrito detalle. ?>
			$.ajax({
				type: "POST",
			...
			<?php if ($edit == 0 && $sw_error == 0 && $dt_ET == 0) { // Recargar condición de pago. ?>
				$.ajax({
					type: "POST",
...
<?php foreach ($array_Dimensiones as &$dim) {
    ...
    if ($DimCode == $DimSeries) {
		...
		<?php foreach ($array_Dimensiones as &$dim) {?>
			...
			<?php $Name_IdDoc = "ID_DevolucionVenta";?>
			...
			$("#<?php echo $dim['IdPortalOne']; ?>").change(function() {
				...
				var docType = 7;
				var detalleDoc = "detalle_devolucion_venta.php";
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							url: "registro.php?P=36&doctype=7&type=1&name=EmpVentas&value="+Base64.encode(document.getElementById('EmpleadoVentas').value)+"&line=0&cardcode="+document.getElementById('CardCode').value+"&whscode=0&actodos=1",
							success: function(response){
								frame.src="detalle_devolucion_venta.php?id=0&type=1&usr=<?php echo $_SESSION['CodUser']; ?>&cardcode="+document.getElementById('CardCode').value;
								$('.ibox-content').toggleClass('sk-loading',false);
							}
						...
						$.ajax({
							type: "GET", // "EmpVentas" es el nombre que tiene el registro en el detalle.
							url: "registro.php?P=36&doctype=7&type=2&name=EmpVentas&value="+Base64.encode(document.getElementById('EmpleadoVentas').value)+"&line=0&id=<?php echo $row['ID_DevolucionVenta']; ?>&evento=<?php echo $IdEvento; ?>&actodos=1",
							success: function(response){
								frame.src="detalle_devolucion_venta.php?id=<?php echo base64_encode($row['ID_DevolucionVenta']); ?>&evento=<?php echo base64_encode($IdEvento); ?>&type=2";
								$('.ibox-content').toggleClass('sk-loading',false);
							}
			...
			if(document.getElementById('PrjCode').value!=""&&document.getElementById('CardCode').value!=""&&document.getElementById('TotalItems').value!="0"){
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=7&type=1&name=PrjCode&value="+Base64.encode(document.getElementById('PrjCode').value)+"&line=0&cardcode="+document.getElementById('CardCode').value+"&whscode=0&actodos=1",
							success: function(response){
								frame.src="detalle_devolucion_venta.php?id=0&type=1&usr=<?php echo $_SESSION['CodUser']; ?>&cardcode="+document.getElementById('CardCode').value;
								$('.ibox-content').toggleClass('sk-loading',false);
							}
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=7&type=2&name=PrjCode&value="+Base64.encode(document.getElementById('PrjCode').value)+"&line=0&id=<?php echo $row['ID_DevolucionVenta']; ?>&evento=<?php echo $IdEvento; ?>&actodos=1",
							success: function(response){
								frame.src="detalle_devolucion_venta.php?id=<?php echo base64_encode($row['ID_DevolucionVenta']); ?>&evento=<?php echo base64_encode($IdEvento); ?>&type=2";
								$('.ibox-content').toggleClass('sk-loading',false);
							}
...
<?php if ($edit == 1) {
    ...
    foreach ($array_Dimensiones as &$dim) {
        ...
        $CopyDim .= "$DimIdPO=$encode_OcrCode&";
				...
				<input type="hidden" form="CrearDevolucionVenta" id="SigCliente" name="SigCliente" value="" />
