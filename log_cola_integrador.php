				...
				<div class="row">
					<div class="col-lg-12">
						<div class="ibox-content">
							...
							<form action="log_cola_integrador.php" method="post" id="formBuscar" class="form-horizontal">
							...
							<div class="col-lg-2">
								<select name="Estado" class="form-control" id="Estado">
										<option <?php if (isset($_POST['Estado']) && $_POST['Estado'] == 2) {echo "selected=\"selected\"";}?> value="2">(Todos)</option>
										<option <?php if (isset($_POST['Estado']) && $_POST['Estado'] == 1) {echo "selected=\"selected\"";}?> value="1">Integrado</option>
										<option <?php if (isset($_POST['Estado']) && $_POST['Estado'] == 0) {echo "selected=\"selected\"";}?> value="0">Pendiente</option>
										<option <?php if (isset($_POST['Estado']) && $_POST['Estado'] == -1) {echo "selected=\"selected\"";}?> value="-1">Error</option>
								</select>
							</div>
...
?>
				<div class="row">
					...
					<div class="row m-b-md">
						<div class="col-lg-12">
							<button class="pull-right btn btn-success" id="btnCambiarLote" name="btnCambiarLote" onClick="CambiarEstadoEnLote();" disabled><i class="fa fa-pencil"></i> Integración Pendiente</button>
						</div>
					</div>
...
if ($sw == 1) {
    while ($row = sqlsrv_fetch_array($SQL)) {?>
        ...
        if ($row['Integracion'] == '1') {echo "class='label label-info'> Integrado";} elseif ($row['Integracion'] == '-1') {echo "class='label label-danger'> Error";} else {echo "class='label label-primary'> Pendiente";}
        echo "</span>"; ?>
													</td>
													<td class="text-center">
										<div class="checkbox checkbox-success" id="dvChkSel<?php echo $row['ID_Cola']; ?>">
											<input type="checkbox" class="chkSelOT" id="chkSelOT<?php echo $row['ID_Cola']; ?>" value="" onChange="SeleccionarOT('<?php echo $row['ID_Cola']; ?>');" aria-label="Single checkbox One"><label></label>
										</div>
													</td>
												</tr>
										<?php }
